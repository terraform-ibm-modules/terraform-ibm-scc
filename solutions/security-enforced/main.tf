#######################################################################################################################
# Parse regions from CRNs to use in provider config
#######################################################################################################################
...
locals {
  ...
  kms_region            = var.existing_kms_instance_crn != null ? module.existing_kms_crn_parser[0].region : var.existing_kms_key_crn != null ? module.existing_kms_key_crn_parser[0].region : 
...
module "scc_da" {
  ...
  prefix = var.prefix
  provider_visibility = local.provider_visibility
